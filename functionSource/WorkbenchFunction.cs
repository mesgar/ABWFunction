...
namespace Workbench.Functions
{
  public static class WorkbenchFunction
  {
    ...
    static int CONSTRUCTOR_ID = 2;
    static int WORKFLOW_FUNCTION_ID = 4;
        ...
        ILogger log)
    {
      ...
      try
      {
        ...
        */
        ...
        // このステップを実行してから
        // AD上にメールアドレスを持つユーザーを招待・作成して、そのユーザーでABWにログインする。
        // この時点で３のユーザーがDBに存在する
        // ユーザーID２はAPI用、ユーザーID３は上記で作ってユーザー
        // ABWでアプリを作成して、上記で作ったユーザーをアサインする
        // トリックとして、DBのRoleAssignmentにあるユーザーID3とユーザーID２を入れ替えていく
        // 本来であればAPIユーザーを事前に作成して、そのユーザーでABWにログインするのが良いが・・・
        // ユーザー作成できない場合上記のトリックでAPIユーザーを作成できる
